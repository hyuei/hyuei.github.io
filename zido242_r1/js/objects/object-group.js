...
ObjectGroup.inherit({
	...
	createOther:function(obj){
		var gotObj = null;
		while(!gotObj){
			var tempObj = game.rnd.pick(this.objects)
			if(tempObj.a != obj){
				gotObj = tempObj.a;
			}
		}

		var x = gotObj.x - (gotObj.width * 0.5);
		var y = gotObj.y;
		var shadow = global.addSprite(x, y, 'ingame/friend-b');
		shadow.anchor.setTo(0.5, 1);
		shadow.x += (shadow.width * 0.3);
		this.gHuntShadow.add(shadow);

		var chara = new OtherChara(shadow.x, shadow.y, 'ingame/friend-a');
		chara.mask = curState().flashlight.lightMask;
		this.gHunt.add(chara)

		this.otherChara = {a:chara, b:shadow};
	},

	checkChara:function(){
		if(!curState().gameStart || curState().gamePaused || curState().gameOver) return;
		var lightBound = curState().flashlight.lightMask.getBounds();
		var lightCircle = new Phaser.Circle(lightBound.centerX, lightBound.centerY, lightBound.width)
		var charaBound = this.secretChara.a.getBounds();
		// var contains = lightBound.containsRect(charaBound);
		var contains = lightCircle.contains(charaBound.centerX, charaBound.centerY)
		if(contains){
			this.caughtChara();
	...
	collectTrash:function(obj){
		...
		var contains = lightCircle.contains(objBound.centerX, objBound.centerY)
		if(contains){
			var idx = null;
